...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!connection_id) {
      throw new Error("connection_id obrigatorio");
    }
    ...
    const url = `https://api.z-api.io/instances/${connection.z_api_instance_id}/token/${connection.z_api_token}/qr-code/image`;
    ...
    return new Response(
      JSON.stringify({ image: data?.value || data?.qrCode || data?.image || data }),
